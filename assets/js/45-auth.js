    ...
    return `
      <div style="background:linear-gradient(135deg,#0d2a4a,#1a0d2e);border:1px solid #ffffff18;border-radius:22px;padding:28px 24px">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px">
          ${['⚽','🏆','🔥','⚡','🦁','🦅','🐺','🎯','🌟','💪'].map(e =>
            `<button class="emoji-btn" onclick="selectEmoji('${e}')" id="emoji-${e}" style="background:#ffffff0d;border:2px solid transparent;border-radius:12px;padding:10px;font-size:22px;cursor:pointer;font-family:inherit">${e}</button>`
          ).join('')}
        </div>
